#!/usr/bin/env bash
# Train Inkling-Small on the IMPOSSIBLE subset of Impossible-LiveCodeBench (oneoff + conflicting),
# logging to Weights & Biases.
#
#   scripts/train_impossiblebench_inkling.sh                        # the config as-is (40 steps)
#   scripts/train_impossiblebench_inkling.sh --set n_steps=2 batch_size=2 group_size=4   # smoke test
#   scripts/train_impossiblebench_inkling.sh --set seed=1 run_name=ib_Inkling-Small_s1   # another seed
#
...
#
# Hyperparameters live in the config (experiments/configs/impossiblebench_inkling_small.json). The one
...
cd "$(dirname "$0")/.."
CONFIG="experiments/configs/impossiblebench_inkling_small.json"
...
echo "==> impossiblebench × Inkling-Small   config=${CONFIG}"
