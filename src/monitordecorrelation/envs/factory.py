...
``hardest1024``, shared with codeforces_rh); MBPP ignores it. ``cfg.env_options`` passes env-specific
knobs (impossiblebench, codeforces_ib, codeforces_rh) straight to the constructor, which validates them.
...
def make_env(cfg: "ExperimentConfig"):
    ...
    if cfg.env_options and cfg.env not in ("impossiblebench", "codeforces_ib", "codeforces_rh"):
        raise ValueError(
            f"env_options is only supported by the impossiblebench, codeforces_ib and codeforces_rh envs, "
            f"not {cfg.env!r}")
    ...
    if cfg.env == "codeforces_rh":
        from monitordecorrelation.envs.codeforces_rh import CodeforcesRhEnv

        return CodeforcesRhEnv.from_dataset(
            subset=cfg.subset, n=cfg.n_prompts_pool, seed=cfg.seed, **(cfg.env_options or {})
        )
