...
def build_datum(tokenizer, rollout: Rollout, advantage: float) -> tinker.Datum:
    ...
    assigns ``advantage`` to every completion token, 0 to prompt tokens. Sequences are right-shifted:
    input = full[:-1], targets = full[1:].
    """
    ...
    advantages = [0.0] * n_prompt + [advantage] * len(comp_ids)
